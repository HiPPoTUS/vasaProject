...
class WalkerBase(MJCFBasedRobot):
	...
	def apply_action(self, a, controlMode='TORQUE'):
...
class Humanoid(WalkerBase):
	...
	def apply_action(self, a, controlMode='TORQUE'):
		...
		if controlMode=='TORQUE':
			force_gain = 1
			for i, m, power in zip(range(a.size), self.motors, self.motor_power):
				m.set_motor_torque(float(force_gain * power * self.power * np.clip(a[i], -1, +1)))
		if controlMode=='POSITION' or controlMode=='VELOCITY':
			for i, j in zip(range(a.size), self.jdict.keys()):
				if controlMode=='POSITION':
					jdict[j].set_position(a[i])
				if controlMode=='VELOCITY':
					jdict[j].set_velosity(a[i])
